...
//  and account deletion. Presented as a sheet from ConferenceHomeView's
//  toolbar (see its account button). Styled like Home's own cards
//  (.polishedCard(), warmBackground) rather than a plain system list.
//  Ported from MMCoach's AccountView -- same structure and guarantees.
//
...
struct AccountView: View {
    ...
    /// ConferenceHomeView gates its account button on `onSignOut` being
    /// present.
    ...
    private var actionsCard: some View {
        VStack(spacing: 0) {
            ...
            settingsRow(icon: "hand.raised", title: "Privacy & AI Data Use") {
        ...
        .confirmationDialog(
            ...
            Text("You'll need to sign back in to access your cases. Your cases and subscription aren't affected.")
        }
    }
    ...
    private func deleteAccountCard(_ action: @escaping () async throws -> Void) -> some View {
        ...
        .confirmationDialog(
            ...
            // the trainee's behalf, so deleting the account here would
        ...
        Button(action: action) {
            HStack(spacing: 12) {
                Image(systemName: icon)
                    ...
                    .foregroundStyle(isDestructive ? Color.red : Color.copilotPrimaryText)
    ...
    private func deleteAccount(using action: @escaping () async throws -> Void) {
        ...
        Task {
            do {
                ...
                // RootView), which tears down ConferenceHomeView -- and
                // this sheet along with it -- the same way Sign Out
                // already works today. No explicit dismiss() needed here.
