...
//
//  The single interface between Heart Team Prep and Apple in-app
//  purchases. Views and view models never call StoreKit directly -- see
//  PaywallViewModel and ConferenceHomeViewModel, the only callers of this
//  protocol, and StoreKitSubscriptionService, the only concrete
//  implementation. Protocol-based so it can be mocked in previews/tests
//  (see PreviewSubscriptionService), mirroring AuthenticationService.
//  Mirrors MMCoach's SubscriptionService -- same shape, same guarantees.
//
...
/// for development testing (see Subscriptions/HeartTeamPrep.storekit).
enum SubscriptionProductID {
    static let monthly = "dev.benderapps.HeartTeamPrep.subscription.monthly"
    static let annual = "dev.benderapps.HeartTeamPrep.subscription.annual"
...
struct SubscriptionPlan: Identifiable, Equatable {
    ...
    /// Localized, formatted total price for the period (e.g. "$9.99") --
...
enum SubscriptionPurchaseOutcome: Equatable {
    case success
    /// The trainee dismissed the purchase sheet -- not an error.
    case userCancelled
...
enum SubscriptionServiceError: LocalizedError, Equatable {
    ...
    var errorDescription: String? {
        ...
        case .network:
            return "Heart Team Prep couldn't reach the App Store. Check your connection and try again."
        case .unknown:
