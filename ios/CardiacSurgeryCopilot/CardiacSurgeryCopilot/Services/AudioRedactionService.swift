...
//
//  See SpeechRecognitionService for where this is actually called.
//
...
enum AudioRedactionService {
    ...
    /// neighboring word. When two PHI items are spoken close together,
    /// padding on both sides of both spans can consume most of the real
    /// (non-PHI) speech between them, leaving only a stray word fragment
    /// in the final transcript -- 0.15s covers meaningful coarticulation
    /// bleed/timestamp imprecision while limiting that risk.
    ...
    /// the word immediately preceding a gap of silence routinely has its
    /// reported duration inflated to cover that trailing silence, pushing
    /// its `end` past where the gap actually starts and causing an
    /// end-based search to skip over it and land the placeholder one word
    /// too early. A segment's `timestamp` (when it starts) isn't subject
    /// to that bias -- it's set by when speech begins, not by how long the
    /// recognizer decides to attribute to it before the next event.
    static func splicePlaceholders<Tag: Hashable>(
        ...
        // very start/end of the transcript.
        var pieces: [String] = []
    ...
    /// a single call can silently stop short of the requested frameCapacity
    /// with no error thrown, and a second call to `read(into:)` on the same
    /// buffer overwrites from frame 0 rather than appending, so naively
    /// looping that would also lose data. This reads in bounded chunks
    /// (with an explicit `frameCount`, which does behave as documented)
    /// into a small scratch buffer and copies each chunk's samples into
    /// the correct offset of the result buffer.
