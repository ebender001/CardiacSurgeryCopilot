...
//  the protocol it implements -- mirrors PreviewAuthenticationService.
//  Used by SwiftUI previews (see PaywallView) and available the same way
//  to a future unit test target: the protocol boundary this mocks is
//  exactly what makes PaywallViewModel/ConferenceHomeViewModel testable
//  without StoreKit.
//
...
extension Array where Element == SubscriptionPlan {
    static let previewPlans: [SubscriptionPlan] = [
        SubscriptionPlan(id: SubscriptionProductID.annual,
                          period: .annual,
                          displayName: "Heart Team Prep Annual",
                          displayPrice: "$9.99"),
        SubscriptionPlan(id: SubscriptionProductID.monthly,
                          period: .monthly,
                          displayName: "Heart Team Prep Monthly",
                          displayPrice: "$1.99"),
    ]
}
#endif
