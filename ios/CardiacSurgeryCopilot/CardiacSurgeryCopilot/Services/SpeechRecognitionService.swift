...
//
//  Owns speech recognition for dictating a case narrative or a follow-up
//  answer. Nothing outside this file talks to Speech/AVFoundation directly.
//
//  PHI hardening: raw audio is recorded locally ONLY. There is no live
//  streaming request to Apple's servers -- the mic is captured to a
//  private temp file and nothing else. Once the trainee stops:
...
final class SpeechRecognitionService: ObservableObject {
    enum ServiceError: LocalizedError, Equatable {
        ...
        var errorDescription: String? {
            ...
            case .recognitionFailed:
                return "Dictation stopped before speech could be transcribed. Please try again, or type instead."
            }
        }
    }
    ...
    /// available rather than leaving the UI waiting indefinitely.
    ...
    /// `medicalDictionary` determines which terms seed contextualStrings
    /// for the re-transcribe pass (see `reTranscribeRedactedAudio`) --
    /// callers should pass the same instance their DictationController
    /// uses, so recognition and correction stay consistent within one
    /// session.
    ...
    func startDictation() async {
        ...
        guard recordingFile != nil else {
            ...
            // opened -- this file is load-bearing, not optional scaffolding.
            self.error = .audioSessionFailure
            teardownAudioSession()
            return
        }
    ...
    ///
    /// `async` specifically for the `Task.yield()` below -- without it,
    /// the session could finish with a fully-correct `sessionTranscript`
    /// that never made it into the app's text field at all, with no error
    /// shown, because DictationController observes `$sessionTranscript`
    /// and `$isRecording` via two SEPARATE `Task { for await ... }` loops;
    /// setting both properties back-to-back with no suspension point
    /// between them does not guarantee the `$sessionTranscript` loop's
    /// iteration (updating its `pendingSessionTranscript`) actually runs
    /// before the `$isRecording` loop's iteration reacts to
    /// `isRecording == false` and reads that value. Yielding after setting
    /// `sessionTranscript` and before setting `isRecording` gives the
    /// transcript-observing task a guaranteed opportunity to run first.
    ...
    private func failDictation() {
        ...
        // once under an interruption -- without this guard, a late
        // callback could overwrite an already-finalized session's state
        // with an error after the pipeline had already completed (via the
        // on-device-transcript fallback) with whatever it had. Mirrors the
        // same guard finishDictation(text:) already has.
        guard isRecording else { return }
        pipelineFallbackTask?.cancel()
        pipelineFallbackTask = nil
        onDeviceRecognitionTask?.cancel()
    ...
    private func runOnDeviceScan(fileURL: URL) {
        ...
        // Confirmed on real hardware (a physical iPhone, on-device file
        // recognition): despite shouldReportPartialResults = false, this
        // callback can still fire more than once for one task -- a
        // non-final callback carrying the real, complete transcript,
        // followed by a "final" callback with an EMPTY transcript, then a
        // cancellation error. Trusting only the literal isFinal-and-non-empty
        // callback (as this used to) discarded real, already-recognized
        // speech. Instead, remember the best (most recent non-empty)
        // transcript across every callback for this task, and only act
        // once the task has truly ended (an error, or a final callback) --
        // using that remembered transcript even if the callback that
        // ended the task carried no text of its own. `hasFinished` guards
        // against acting twice when, as observed, an empty final callback
        // is followed by a cancellation callback for the same task.
        var bestTranscript: (text: String, segments: [SFTranscriptionSegment])?
        ...
        onDeviceRecognitionTask = onDeviceRecognizer.recognitionTask(with: request) { [weak self] result, taskError in
            ...
            #endif
            if let result, !result.bestTranscription.formattedString.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                bestTranscript = (result.bestTranscription.formattedString, result.bestTranscription.segments)
            }

            guard taskError != nil || result?.isFinal == true else { return }
            guard !hasFinished else { return }
            hasFinished = true

            guard let (transcript, segments) = bestTranscript else {
                try? FileManager.default.removeItem(at: fileURL)
    ...
    /// actor -- a deliberate tradeoff: it's chunked file I/O over a few MB
    /// at most for a realistic dictation segment (fast on-device), the
    /// pipeline's actual latency is dominated by the network calls on
    /// either side of it, and the UI is already showing a wait state for
    /// the whole pipeline regardless. If redaction or the re-transcribe
    /// upload can't run at all, falls back to the on-device transcript
    /// (text-redacted) rather than losing the dictation outright.
    private func redactAndReTranscribe(sourceURL: URL, transcript: String, segments: [SFTranscriptionSegment]) async {
        ...
        // available -- when it isn't, SFSpeechRecognizer silently falls
        // back to on-device for this request too (taskError stays nil,
        // isFinal still fires normally) rather than erroring --
        // contextualStrings just has no effect for that call, same as if
        // it had been requested on-device directly. The explicit
        // `taskError != nil` fallback just below this still matters for a
        // genuine failure (e.g. a request that reaches the server but the
        // server errors), not for "offline" specifically. The audio it
        // receives has already had every flagged span muted.
        reTranscribeRequest.contextualStrings = medicalDictionary.contextualStringSeed

        // Same real-hardware quirk as runOnDeviceScan above -- track the
        // best non-empty result seen across every callback for this task
        // (an empty *genuine* final result is still legitimate here, per
        // the doc comment above, so this only protects against a later
        // empty/canceled callback discarding an earlier GOOD one, not
        // against a truly empty result winning when nothing better ever
        // arrived) -- plus a `hasFinished` guard against acting twice.
        var bestReTranscribedResult: SFSpeechRecognitionResult?
        var hasFinishedReTranscribe = false
        ...
        reTranscribeTask = onDeviceRecognizer.recognitionTask(with: reTranscribeRequest) { [weak self] result, taskError in
            ...
            #endif
            if let result, !result.bestTranscription.formattedString.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                bestReTranscribedResult = result
            }

            guard taskError != nil || result?.isFinal == true else { return }
            guard !hasFinishedReTranscribe else { return }
            hasFinishedReTranscribe = true

            try? FileManager.default.removeItem(at: redactedURL)

            if let usable = bestReTranscribedResult ?? (taskError == nil ? result : nil) {
                Task { @MainActor in
                    let final = Self.splicedAndReScreened(reTranscribedResult: usable, spans: spans)
    ...
    private static let recordingFilePrefix = "cardiacsurgerycopilot-dictation-"
