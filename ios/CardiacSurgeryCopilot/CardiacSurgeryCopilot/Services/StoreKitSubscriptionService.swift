...
//  app-wide singleton rather than something created per-view: it owns the
//  one long-lived Transaction.updates listener for the app's lifetime, so
//  creating more than one instance would mean redundant listeners.
//  Ported from MMCoach's StoreKitSubscriptionService -- same behavior,
//  same timeout/verification/entitlement-caching guarantees.
//
