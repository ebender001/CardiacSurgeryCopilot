...
//  PasswordResetView) and the app-root decision between them and the
//  signed-in app (see RootView). Views only ever call this -- never
...
final class AuthenticationViewModel: ObservableObject {
    ...
    /// `.cscSessionExpired` in BackendService) -- not on an explicit Sign
    /// Out. Shown on WelcomeView so landing back there unexpectedly
    ...
    init(authService: AuthenticationService? = nil,
        ...
        sessionExpiryObservation = NotificationCenter.default
            .publisher(for: .cscSessionExpired)
    ...
    /// fail from the UI's point of view -- there's nothing to stay
    /// on-screen for. This is what gives RootView a spinner to show
    /// immediately instead of leaving the signed-in app visible and inert
    /// for however long the logout call takes.
    func signOut() async {
        state = .endingSession
    ...
    /// the caller stays on-screen to show the error and let the person
    /// retry. Only a success moves to `.endingSession` for a deliberate
    /// beat -- long enough for RootView's spinner to actually be visible
    /// -- before `.signedOut`, so a successful deletion still gets the
    /// same graceful transition as sign-out rather than an instant cut
    /// away from the signed-in app.
    ...
    /// rather than leaving the trainee stuck retrying actions that can
    /// never succeed with a dead session.
