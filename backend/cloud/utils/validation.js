...
/**
 * Every case-touching Cloud Function must resolve the owner from the
 ...
 * `CARDIACSURGERYCOPILOT_ADMIN_SECRET`, never against a Parse.User session.
 * Fails closed if the env var isn't configured, rather than treating an
 * unset secret as "no check needed".
 */
function requireAdminSecret(request) {
  const configured = process.env.CARDIACSURGERYCOPILOT_ADMIN_SECRET;
