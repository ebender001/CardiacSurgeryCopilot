...
function identificationParams() {
  ...
  if (email) params += `&tool=CardiacSurgeryCopilot&email=${encodeURIComponent(email)}`;
  return params;
}
 ...
 * Only the fields the client actually displays are extracted --
 * unrecognized tags are ignored rather than causing a parse failure.
 */
 ...
 * PubMed's own relevance sort (no re-ranking). Returns `[]` if nothing is
