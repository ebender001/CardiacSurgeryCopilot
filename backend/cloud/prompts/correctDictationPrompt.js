...
/**
 * Builds the prompt used to clean up one freshly-dictated segment of a
 * cardiac surgery case narrative. Only the new segment is corrected --
 * prior narrative is passed along purely as context, not something the
 * model should re-edit, so already-reviewed text never gets silently
 * rewritten a second time.
 */
function buildCorrectDictationPrompt({ priorNarrative, newSegment }) {
  const system = `You are cleaning up one segment of speech-to-text output from a cardiac surgery trainee dictating a case narrative.
