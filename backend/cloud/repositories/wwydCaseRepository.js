 ...
 * CSCWwydCase. Everything else works with plain JSON case objects.
 *
 * Uses the master key because Cloud Code is the only path allowed to
 * read/write CSCWwydCase -- Class-Level Permissions deny direct client
 * REST/SDK access. Master key use means Parse's own ACL enforcement never
 * runs here, so per-user ownership is enforced explicitly in
 * services/wwydCaseService.js (it compares `request.user.id` against the
 * `ownerId` this module returns) rather than relied upon implicitly. The
 * ACL this module still sets on every case is defense-in-depth for if CLP
 * is ever loosened later, not the primary access-control mechanism.
 */
const CASE_CLASS_NAME = 'CSCWwydCase';

function getWwydCaseClass() {
...
function toClientJSON(parseObject) {
  ...
  return {
    ...
    casePresentation: parseObject.get('casePresentation') || null,
    startingQuestion: parseObject.get('startingQuestion') || null,
...
async function create({ ownerId, ...record }) {
  const WwydCase = getWwydCaseClass();
  const parseObject = new WwydCase();
 ...
 * to an existing CSCWwydCase (including malformed ids) -- callers treat
 * both the same way: case not found.
 */
async function getById(caseId) {
  const WwydCase = getWwydCaseClass();
  const query = new Parse.Query(WwydCase);
  try {
...
/**
 * Adds to CSCWwydCase's running AI-cost/token totals -- called once per AI
 * provider call (see wwydCaseService.js), separately from `update()`'s
 * patch-based writes since this needs a true atomic increment rather than
 * a read-then-overwrite of a JS number, which would lose an update if two
 * AI calls for the same case ever finished close together. `costUSD` may
 * be `null` (unpriced model, see `config/aiPricing.js`), in which case
 * only the token total advances.
 */
 ...
 * truth for the client's WWYD "recent discussions" list. Returns full case
 * JSON (same shape as getById); callers that only need list-row fields
 * pick those out themselves rather than this trimming the shape, so
 * there's one case JSON shape everywhere.
 */
...
/** Number of WWYD cases owned by a user, regardless of status. */
...
/**
 * Permanently removes every WWYD case a user owns -- part of account
 * deletion. Does not touch CSCCaseAICost rows; the caller is responsible
 * for also clearing those via aiCostRepository.deleteAllForOwner, since
 * this module never touches that class.
 */
