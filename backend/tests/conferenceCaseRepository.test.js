...
describe('conferenceCaseRepository', () => {
  ...
  it('incrementAIUsage adds to (not replaces) the running totals across multiple calls', async () => {
    ...
    await conferenceCaseRepository.incrementAIUsage(created.objectId, { costUSD: 0.01, totalTokens: 150 });
    await conferenceCaseRepository.incrementAIUsage(created.objectId, { costUSD: 0.02, totalTokens: 100 });

    const fetched = await conferenceCaseRepository.getById(created.objectId);
  ...
  it('incrementAIUsage advances the token total even when costUSD is null (unpriced model)', async () => {
    ...
    await conferenceCaseRepository.incrementAIUsage(created.objectId, { costUSD: null, totalTokens: 75 });
  ...
  it('returns null for a missing or malformed case id', async () => {
    expect(await conferenceCaseRepository.getById('does-not-exist')).toBeNull();
  });

  it('updates only the given fields and preserves the rest', async () => {
    const created = await conferenceCaseRepository.create(baseRecord());
    const updated = await conferenceCaseRepository.update(created.objectId, { status: 'ready_to_finalize' });

    expect(updated.status).toBe('ready_to_finalize');
    expect(updated.originalNarrative).toBe('narrative text');
    expect(updated.extractedCase).toEqual({ diagnosis: 'Severe aortic stenosis' });
  });

  it('defaults referenceLookups to an empty object and report to null', async () => {
    const created = await conferenceCaseRepository.create(baseRecord());
  ...
  describe('listForOwner', () => {
    ...
    it('returns only the given owner\'s cases, most recent first', async () => {
      ...
      await conferenceCaseRepository.create(baseRecord({ ownerId: 'listforowner-user', originalNarrative: 'first' }));
      await wait();
      await conferenceCaseRepository.create(baseRecord({ ownerId: 'listforowner-other', originalNarrative: 'not this one' }));
      await wait();
      await conferenceCaseRepository.create(baseRecord({ ownerId: 'listforowner-user', originalNarrative: 'second' }));

      const cases = await conferenceCaseRepository.listForOwner('listforowner-user');
    ...
    it('returns an empty array for an owner with no cases', async () => {
      expect(await conferenceCaseRepository.listForOwner('listforowner-nobody')).toEqual([]);
    });
  });
});
